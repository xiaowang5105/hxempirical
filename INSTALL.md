...
1.6.2 增加完整 CSV 检查、模型选择与 TSV 导出、合并预检和项目 ZIP；保留 1.6.1 的转换和载入保护。更新时继续使用下述唯一安装入口。批量覆盖会先展示冲突清单；转换后载入需要确认替换当前数据，载入失败保留原状态。项目导出重放会重新生成转换输出，请检查原文件路径和输出位置。

1.6.0 的 9 月 6 日修订补充项目恢复保护。更新前请保存项目，并把 `.hxproj`、`.hxproj.recovery`、`.hxproj.bak` 和旁边的 `hx-assets-*` 一起备份；项目当前支持单 frame。完整使用限制见 [研究项目说明](docs/PROJECTS.md)。

## 先选安装方式

面板重放与 IV 设置修订适用于新记录；旧项目中漏记的 `xtset`、内生变量和工具变量需重新核对。更新后的单元格编辑使用原始值，显示格式继续用于表格展示。
