...
EXPECTED: dict[str, set[str]] = {
    ...
    "时间序列": {"arima", "arfima", "arimasoc", "arfimasoc", "newey", "prais", "arch", "ucm", "mswitch", "threshold", "dfgls", "dfuller", "pperron", "corrgram", "cumsp", "pergram", "wntestb", "wntestq", "psdensity", "rolling", "forecast", "tsappend", "tsfill", "tsfilter", "tsreport", "tssmooth"},
    "多元时间序列": {"var", "varsoc", "vargranger", "varlmar", "varnorm", "varstable", "irf", "lpirf", "svar", "vec", "vecrank", "veclmar", "vecnorm", "vecstable", "varbasic", "varwle", "mgarch", "dfactor", "sspace", "xcorr"},
    "空间自回归模型": {"spregress", "spivregress", "spxtregress"},
    "纵向/面板数据": {"xtreg", "xtlogit", "xtprobit", "xtologit", "xtpoisson", "xtnbreg", "xtgee", "xttobit", "xtcloglog", "xtintreg", "xtoprobit", "xtmlogit", "xtfrontier", "xtivreg", "xtpcse", "xtgls", "xtregar", "xtrc", "xtstreg", "xteregress", "xteprobit", "xteoprobit", "xteintreg", "xtheckman", "xthtaylor", "xtabond", "xtdpdsys", "xtdpd", "xtunitroot", "xtcointtest", "xtdescribe", "xtsum", "xttab", "xtdata"},
    "多层混合效应模型": {"mixed", "mecloglog", "melogit", "meprobit", "mepoisson", "menbreg", "meologit", "meoprobit", "meintreg", "menl", "mestreg", "metobit", "meglm"},
    "生存分析": {"stset", "stcox", "streg", "stintreg", "stintcox", "stcrreg", "sts", "stcurve", "stdescribe", "stsum", "stci", "stbase", "stfill", "stgen", "stsplit", "stvary", "sttocc", "sttoct", "stir", "strate", "stptime", "stmh", "stmc", "ctset", "cttost", "ltable", "snapspan"},
    "流行病学及相关": {"cc", "cs", "ir", "mcc", "dstdize", "pkexamine", "pksumm", "pkcross", "pkequiv", "pkcollapse", "pkshape"},
    "内生协变量": {"eregress", "eprobit", "eoprobit", "eintreg"},
    "因果推断/处理效应": {"didregress", "xtdidregress", "hdidregress", "xthdidregress", "mediate", "teffects", "eteffects", "etregress", "etpoisson", "stteffects", "telasso"},
    "结构方程模型(SEM)": {"sem", "gsem"},
    "潜在类别分析(LCA)": {"gsem"},
    "有限混合模型(FMM)": {"fmm"},
    "项目反应理论(IRT)": {"irt", "irtgraph", "diflogistic", "difmh"},
    "DSGE模型": {"dsge", "dsgenl"},
    "多元分析": {"alpha", "factor", "pca", "canon", "ca", "candisc", "hotelling", "manova", "mvreg", "mca", "mds", "mdslong", "mdsmat", "mvtest", "procrustes", "discrim", "cluster"},
    "调查数据分析": {"svyset", "svydescribe", "svy"},
    "Lasso回归": {"lasso", "elasticnet", "sqrtlasso", "poregress", "pologit", "popoisson", "dsregress", "dslogit", "dspoisson", "poivregress", "xporegress", "xpologit", "xpopoisson", "xpoivregress"},
    "Meta分析": {"meta"},
    "多重插补": {"mi"},
    ...
    "贝叶斯分析": {"bayes", "bayesmh", "bayespredict", "bayesreps", "bayesstats", "bayesgraph", "bayestest", "bayesvarstable", "bayesirf", "bayesfcast"},
    ...
    "估计后分析": {"test", "testparm", "testnl", "lincom", "nlcom", "contrast", "pwcompare", "predict", "predictnl", "margins", "marginsplot", "lrtest", "hausman", "suest", "linktest", "estimates", "estat"},
}
...
def method_block(method: str) -> str:
    ...
    return REGISTRY[line_start:min(candidates) if candidates else len(REGISTRY)]
...
for method, expected in EXPECTED.items():
    ...
    if actual != expected:
        fail(f"{method} routing drift; missing={sorted(expected-actual)}, extra={sorted(actual-expected)}")
