...
# Category overviews were written into an orphan panel while the visible scroll
# pane was bound to chooserResultsHost.  Use one visible host for both levels.
s = replace_once(
    s,
...
# Stats carried a duplicated category-finish block. Reuse the shared finish path.
s = replace_once(
    s,
...
# Never let a missing runtime characteristic blank the navigation.  The shipped
# Java catalog is the baseline; live hxregistry values may append additions.
...
for name in ["hxempirical.ado", "hxempirical.sthlp", "hxinstaller.ado", "hxinstall.do", "README.md", "INSTALL.md"]:
    p = Path(name)
    t = p.read_text(encoding="utf-8").replace("1.5.2", "1.5.3")
...
nt = readme.read_text(encoding="utf-8")
marker = "### 安装或更新\n"
