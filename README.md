...
2. 在工作台运行分析；需要保存进度时，选择“保存项目和当前数据”。
...
1.6.0 已在 Windows / StataNow 19.5 MP 上通过 11 项真实 Stata 测试，并在 GitHub Actions 的 Ubuntu 环境通过 Java 11 编译、项目行为和发布一致性检查。Stata 17/18 与 macOS 尚未在本次验证中实测。
