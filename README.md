...
安装器使用当前用户的 `PERSONAL` ado 目录，先完整下载并校验发布清单，再统一写入正式目录；更新发生写入错误时会恢复原有文件。普通用户只需要记住这一条命令：未安装时执行安装，检测到已有安装时自动执行安全更新。成功后会建立唯一的 **用户（User） > 我的实证工具箱** 菜单入口。
