...
2026-09-06 的修订已在 Windows / StataNow 19.5 MP 上通过全部 12 项真实 Stata 测试，以及 Java 项目行为和 6 项仓库／发布检查。新增回归测试覆盖 IV 控件恢复、面板声明与脚本重放、单元格原值编辑和嵌套引号。详细范围见[验证记录](docs/VALIDATION-1.6.0.md)，当前分支的远端检查及网页部署状态见 [GitHub Actions](https://github.com/xiaowang5105/hxempirical/actions)。Stata 17/18 与 macOS 尚未在本次环境中实测。
